...
if (isPost()) {
    ...
    if ($action === 'check_email') {
        ...
        $db = getDB();
        $stmt = $db->prepare('SELECT id, nama FROM pegawai WHERE email = ? LIMIT 1');
        $stmt->execute([$email]);
        $user = $stmt->fetch();
        if ($user) {
            $step = 2;
            ...
            $error = 'Email tidak terdaftar sebagai Pegawai.';
        }
        ...
        if (strlen($pass) < 8) {
            ...
            $step = 2;
            $db = getDB();
            $stmt = $db->prepare('SELECT nama FROM pegawai WHERE id = ? LIMIT 1');
            ...
            $stmt = $db->prepare('UPDATE pegawai SET password_hash = ? WHERE id = ?');
...
<head>
  ...
  <title>Lupa Sandi Pegawai — Sistem Manajemen Audit</title>
...
<div class="page">
  ...
  <main class="card" role="main">
    ...
    <h1 class="card-title">Lupa Sandi Pegawai</h1>
    ...
    <form method="POST" action="">
      ...
      <div class="field">
        <div class="field-header">
          <label for="email">Masukkan Email Pegawai</label>
        </div>
        <div class="input-wrap">
          ...
          <input type="email" id="email" name="email" placeholder="email@domain.com" value="<?= e($email) ?>" required autofocus autocomplete="email"/>
        </div>
      </div>
      ...
      <div style="text-align:center; margin-top: 16px;">
        <a href="login.php" class="forgot">Kembali ke Login</a>
      </div>
    </form>
    ...
    <form method="POST" action="">
      ...
      <div style="background:#F1F5F9; padding:12px; border-radius:10px; margin-bottom:20px; font-size:13px; color:#334155; border:1px solid #E2E8F0; text-align:center">
        Pegawai ditemukan: <br><strong><?= e($user_nama) ?></strong> (<?= e($email) ?>)
      </div>
      ...
      <div style="text-align:center; margin-top: 16px;">
        <a href="login.php" class="forgot">Batal</a>
      </div>
    </form>

    <?php elseif ($step === 3): ?>
    <div style="text-align:center; margin-top:10px">
      <a href="login.php" class="btn-submit" style="display:inline-flex; align-items:center; justify-content:center; text-decoration:none">
        <span class="btn-inner">
          Kembali Login
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M5 12h14M12 5l7 7-7 7"/>
